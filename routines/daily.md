...
CAPTURE the SOURCE too (source-discovery — the radar grows its own coverage). The same capture applies to WHERE a primary came from, not just to the item. When any lane names an on-axis primary whose publishing org/domain is NOT already in any SOURCES.md swept list, append/increment that org in SOURCES.md → "Discovered-source candidates" (a tally line: domain — times seen — last on-axis artifact + date — first-seen date). This is a tally only — NO extra fetch, you already hold the URL from routing the item. Sources are discovered exactly like items and curators: the daily STAGES, the weekly VERIFIES and PROMOTES (§ source review). This is what lets the radar find new labs/vendors/repos on its own instead of waiting for the curator — an on-axis lab that announces only on its blog must surface as a staged candidate the first run any lane names it, never sit unrecognized because no swept list happens to point at it.
