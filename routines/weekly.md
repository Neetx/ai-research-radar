...
- Every run: compute the calibration metrics (queue funnel, evidence and stage moves, exploration-slot compliance, off-axis rate, discovery lag by channel) from the week's daily reports and the TRENDS.md git history; append one dated line to the self-eval log `logs/calibration.md` (append-only; the `calibration` section of TRENDS.md is now just a pointer — never write metrics into TRENDS.md).
