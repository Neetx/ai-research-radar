...
**Since last scan (2026-09-17):**

- **New evidence**: [agent security](TRENDS.md#id-agent-security-004-agent-security-formal-limits-of-prompt-injection-defenses-and-the-architectural-turn) gets OpenAI's new [systematic misalignment-disclosure framework](https://openai.com/index/model-misalignment-reporting-framework) (six concrete reports) plus Cohere's [hardware-rooted confidential-computing vault](https://docs.cohere.com/docs/encrypted-vault-overview).
- **New evidence**: [agent harness/runtime infra](TRENDS.md#id-agent-runtime-015-agent-harnessruntimememory-as-a-first-class-engineered-self-improving-object) gets an independent academic confirmation, [HarnessTax](https://harnesstax.github.io/), that harness choice barely moves task success but can move cost 5x; [verifiable RL environments](TRENDS.md#id-rl-env-005-verifiable-rl-environments-as-an-infrastructure-category-for-agent-training) gets [ScienceIDE](https://arxiv.org/abs/2609.19134), turning scientific codebases into agent-learnable environments.
- **Housekeeping**: [watchlist](TRENDS.md#observation_queue) burned down 7 stale items, added 6 below-bar signals; no dormancy crossings — [parametric injection](TRENDS.md#id-parametric-injection-018-parametric-injection-compiling-behavior-and-knowledge-into-model-weights-instead-of-promptcontext) is the closest at 20 days.
- **Study pick**: [HarnessTax](https://harnesstax.github.io/) — a concrete measurement of what a coding agent's default harness actually costs you.
...
| [Agent harness/runtime/memory infra](TRENDS.md#id-agent-runtime-015-agent-harnessruntimememory-as-a-first-class-engineered-self-improving-object) | 🌊 mainstreaming | [2026-09-16](https://harnesstax.github.io/) |
...
- No front-page "earthquake" today — the densest technical threads were [an independent Postgres-query-plan RL write-up](https://rohanbansal.com/qorl) and [an independent harness-matters study](https://harnesstax.github.io/), both well above the noise floor but on already-tracked axes.
- news.smol.ai (Latent.Space/AINews) stays back online (HTTP 200) but content is still stuck stale — treated as degraded pending a fresh digest.
...
- Tooling note: the GitHub external API remains scope-blocked for a 26th+ consecutive week (already notification-flagged); direct RSS/Atom fetches and the `r.jina.ai` proxy continue to cover the lab-sweep and curator lanes without it.

## Output map

[TRENDS.md](TRENDS.md) · [watchlist (~28)](TRENDS.md#observation_queue) · [reports/](reports/) → [2026-09-17](reports/2026-09-17.md) · weekly: [2026-W37](reports/weekly/2026-W37.md) · [AGENTS.md](AGENTS.md) · [SOURCES.md](SOURCES.md)
