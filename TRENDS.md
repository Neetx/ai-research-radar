...
- notes: Three independent groups converge on the same conclusion: filters/wrappers are not enough and security must move into the architecture (least privilege, sandboxing, design patterns). The impossibility results (May 2026) are the strongest recent signal. Cross-link: agent-sandbox-007 is the infrastructure side of this same conclusion. Dormant as of 2026-06-13 (W24 weekly): last_evidence 2026-05-17, 27 days ago; POISE (arXiv 2606.07943, single-group offensive attack) and grammar-jailbreak (2606.11817) in queue but are attack papers, not defense/architectural findings — they do not advance the trend thesis. Watch: vendor reactions (platform-level defenses) and primary advisories on real agent vulnerabilities in production; either would reactivate.
...
- notes: Environments with verifiable outcomes are becoming a market/category of their own (sold: Bugcrowd; federated: Prime Intellect; industrialized into training: NVIDIA). Emerging: the pieces are real but cross-lab adoption is not yet measurable from here. Research-velocity update (2026-06-12 exploration pass, via HF daily papers): three more independent works in ~2 weeks — a 63-page systematization survey (CASIA), recursive environment composition (RACES, ISCAS) and co-evolving policy+harness training (EvoTrainer, Alibaba) — plus now-verified GenEnv (Princeton/Mengdi Wang line, 2025-12-22, promoted from the queue on 2026-06-12); a "co-evolve environments with the policy" sub-theme is forming, with two independent groups so far (EvoTrainer Alibaba-line, GenEnv Princeton-line). Independence caveat: the survey (CAS Inst. of Automation) and RACES (CAS Inst. of Software) share the parent academy. Kept at emerging deliberately: this surge is academic, not the cross-lab production adoption that the promotion gate (used for MCP/PD-disagg/lowbit-quant) requires. Watch: NeMo Gym, Environments Hub growth, possible standard interfaces between environments and trainers.
...
- 2026-06-13 — series of real agent vulnerabilities disclosed in Q1 2026 (EchoLeak, GeminiJack, Reprompt…) cited only by secondary sources — still unverified as of 2026-06-13 weekly run; Tavily search found no primary advisories; keep watching — trace the original CVEs or lab advisories next week
...
- 2026-06-13 — [LT2: Linear-Time Looped Transformers](https://arxiv.org/abs/2605.20670) — looping uniquely synergizes with subquadratic attention; Ouro-hybrid-1.4B beats industry 1B models at linear-time cost — the practical path to scalable looped LMs
- 2026-06-12 — [PPD Disaggregation for Multi-turn LLM Serving](https://arxiv.org/abs/2603.13358) — ICML 2026; the "append-prefill is an order of magnitude cheaper than full prefill" insight reframes prefill/decode routing for chat and agent workloads — required reading for anyone running PD-disaggregated serving
- 2026-06-12 — [Agentic Environment Engineering: A Survey](https://arxiv.org/abs/2606.12191) — 63-page systematization of the environment lifecycle (modeling/synthesis/evaluation/application); the map to read before building or buying RL-environment infrastructure
- 2026-06-12 — [On Subquadratic Architectures: From Applications to Principles](https://arxiv.org/abs/2606.12364) — clean head-to-head of xLSTM vs Mamba-2 vs Gated DeltaNet on code-model pretraining, distillation and time-series pretraining, with a unified formulation; orientation for anyone weighing attention alternatives
